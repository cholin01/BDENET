...
class MolEncoder(nn.Module):
    ...
    def embed_bond_feature(self, fmess):
        if self.use_feature:
            # 🌟 核心修复：将用作索引的浮点数列强转为 .long()
            fmess1 = self.E_b.index_select(index=fmess[:, 2].long(), dim=0)
            fmess2 = self.E_fr.index_select(index=fmess[:, 3].long(), dim=0)
            fmess3 = self.E_fc.index_select(index=fmess[:, 4].long(), dim=0)
            fmess4 = self.E_fa.index_select(index=fmess[:, 5].long(), dim=0)

            hmess = torch.cat((fmess1, fmess2, fmess3, fmess4), dim=1)
        else:
            # 同样修复这里
            hmess = self.E_b.index_select(index=fmess[:, 2].long(), dim=0)
