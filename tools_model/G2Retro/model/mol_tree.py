...
class MolTree(object):
    ...
    def __init__(self, smiles, mol=None, use_brics=False, isTest=False, decompose_ring=False, bde_info=None):
        self.smiles = Chem.MolToSmiles(Chem.MolFromSmiles(smiles))
        self.mol = get_mol(self.smiles)

        # Store the dictionary
        self.bde_info = bde_info if bde_info is not None else {}
    ...
    def build_mol_graph(self):
        ...
        for bond in mol.GetBonds():
            ...
            # Lookup BDE using map numbers
            map1 = bond.GetBeginAtom().GetAtomMapNum()
            map2 = bond.GetEndAtom().GetAtomMapNum()
            key1 = map1 if map1 != 0 else a1
            key2 = map2 if map2 != 0 else a2

            # Retrieve BDE value, default to 0.0 if not found
            bde_val = self.bde_info.get((key1, key2), 0.0)
            ...
            # Attach BDE to the edge
            graph[a1][a2]['bde'] = bde_val
            graph[a2][a1]['bde'] = bde_val
    ...
    @staticmethod
    def tensorize_graph(graph_batch, vocab, tree=True, tree_buffer=None, atom_num=1, use_atomic=False, use_brics=False,
                        extra_len=0, use_feature=False):
        if tree:
            fmess = [(0, 0, 0)]
            fnode = [0]
            ...
            # 🌟 修改点：末尾加入 0.0 用于 BDE 浮点数占位
            fmess = [(0, 0, 0, 0, 0, 0, 0, 0.0)]
            if not use_atomic:
                fnode = [(0, 0, 0, 0, 0, 0)]
            else:
                fnode = [(0, 0, 0, 0, 0, 0, 0)]
        ...
        for bid, (G, mol) in enumerate(graph_batch):
            ...
            for u, v, attr in G.edges(data='label'):
                if tree:
                    ...
                    # 🌟 修改点：直接追加真实的浮点数 BDE
                    fmess.append(
                        (u, v, attr, G[u][v]['is_conju'], G[u][v]['in_ring'], G[u][v]['is_aroma'], G[u][v]['dir'],
                         G[u][v]['bde']))
                else:
        ...
        # 🌟 致命细节修复：包含 BDE 浮点数的 fmess 必须声明为 FloatTensor
        if use_feature and not tree:
