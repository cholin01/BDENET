...
def make_cuda(tensors, product=True, ggraph=None):
    ...
    # 🌟 修改点：解绑一刀切。索引 1 为 fmess 转 float，其余转 long
    new_graph_tensors1 = []
    for i, x in enumerate(graph_tensors[:6]):
        if i == 1:
            new_graph_tensors1.append(make_tensor(x).to(device).float())
        else:
            new_graph_tensors1.append(make_tensor(x).to(device).long())

    if len(graph_tensors) > 8:
        new_graph_tensors2 = [graph_tensors[-3], make_tensor(graph_tensors[-2]).to(device),
                              make_tensor(graph_tensors[-1]).to(device)]
...
class MolCenter(nn.Module):
    ...
    def __init__(self, vocab, avocab, args):
        ...
        if self.use_feature and not self.use_mess:
            self.U_t1 = nn.Sequential(nn.Linear(1 + BOND_SIZE + IS_RING_NUM + IS_CONJU_NUM + IS_AROMATIC_NUM + self.hidden_size * 2, self.hidden_size),
                                   nn.ReLU(),
                                   nn.Linear(self.hidden_size, self.hidden_size)).to(device)
        elif not self.use_mess:
            self.U_t1 = nn.Sequential(nn.Linear(1 + BOND_SIZE + self.hidden_size * 2, self.hidden_size),
                                   nn.ReLU(),
                                   nn.Linear(self.hidden_size, self.hidden_size)).to(device)
        
        elif self.use_feature and self.use_mess:
            self.U_t1 = nn.Sequential(nn.Linear(1 + BOND_SIZE + IS_RING_NUM + IS_CONJU_NUM + IS_AROMATIC_NUM + self.hidden_size * 4, self.hidden_size),
                                   nn.ReLU(),
                                   nn.Linear(self.hidden_size, self.hidden_size)).to(device)
        else:
            self.U_t1 = nn.Sequential(nn.Linear(1 + BOND_SIZE + self.hidden_size * 4, self.hidden_size),
                                   nn.ReLU(),
                                   nn.Linear(self.hidden_size, self.hidden_size)).to(device)
    ...
    def convert_bonds(self, bond_type_labels, cand_atom1_vecs, cand_atom2_vecs):
        ...
        if self.use_feature:
            bond_types = self.E_b.index_select(index=bond_type_labels[:, 0].long(), dim=0)
            bond_conju = self.E_fc.index_select(index=bond_type_labels[:, 1].long(), dim=0)
            bond_ring = self.E_fr.index_select(index=bond_type_labels[:, 2].long(), dim=0)
            bond_aroma = self.E_fa.index_select(index=bond_type_labels[:, 3].long(), dim=0)

            # 🌟 修复点：在这里进行简单的归一化缩放
            bde_feature = (bond_type_labels[:, 5] / 100.0).unsqueeze(1)

            bond_types = torch.cat((bond_types, bond_conju, bond_ring, bond_aroma, bde_feature), dim=1)
        else:
            bond_types = self.E_b.index_select(index=bond_type_labels[:, 0].long(), dim=0)
            ...
            bond_types = torch.cat((bond_types, bde_feature), dim=1)
    ...
    def predict_centers(self, product_bond_tensors, product_embed_vecs, product_atom_vecs, product_mess_vecs, product_trees, product_graph_scopes, product_orders):
        ...
        for i, tree in enumerate(product_trees):
            ...
            one_dir_bond_idxs = torch.where(bond_tensor[:, -2] == 0)[0]
        ...
        cand_bond_atom_idxs = torch.cat(cand_bond_atom_idxs, dim=0).to(device).long()
    ...
    def validate_centers(
        ...
        sample_ids=None,
        product_smiles_list=None,
    ):
        """
        Validate reaction-center prediction.

        Original return:
            top_10_acc, bond_center_acc, atom_center_acc

        New optional return when return_details=True:
            top_10_acc, bond_center_acc, atom_center_acc, candidate_details

        candidate_details is a list of dictionaries. Each row corresponds to one
        candidate reaction center and contains candidate type, candidate atoms,
        model score/rank, true-center label, and BDE-related bond features.
        This table is designed for downstream CSV export and figure analysis.
        """
        ...
        product_embed_vecs, product_atom_vecs, product_mess_vecs = self.encode(
            product_tensors,
            classes=react_cls,
        ...
        # Extra metadata for reconstructing candidate rows per molecule.
        detail_scopes = []
        ...
        for i, tree in enumerate(product_trees):
            ...
            bond_size = tree.mol_graph.number_of_edges()
            atom_size = tree.mol_graph.number_of_nodes()

            bond_tensor_full = product_bond_tensors[bonds_offset:bonds_offset + bond_size, :]
            one_dir_bond_tensor = torch.where(bond_tensor_full[:, -2] == 0)[0]

            bond_tensor = bond_tensor_full[one_dir_bond_tensor, :]
            cand_bond_size = bond_tensor.shape[0]

            # True center label in the flattened candidate space:
            #   0 .. 4*cand_bond_size-1: bond center candidates
            #   4*cand_bond_size .. end: atom center candidates
            true_center_type = "atom"
            true_atom_i = None
            true_atom_j = None
            true_bond_local_idx = None
            true_bond_change_type = None

            if bond_order[0][1] == -1:
                true_atom_i = int(bond_order[0][0] - atoms_offset)
                true_label = 4 * cand_bond_size + true_atom_i
                labels.append(true_label)
                atom_center_idxs.append(i)
                true_center_type = "atom"
            else:
                true_bond_local_idx = torch.where(one_dir_bond_tensor == bond_order[0][0] - bonds_offset)[0].item()
                true_bond_change_type = int(bond_order[0][1])
                true_label = 4 * true_bond_local_idx + true_bond_change_type
                labels.append(true_label)
                bond_center_idxs.append(i)
                true_center_type = "bond"

                try:
                    true_pair = bond_tensor[true_bond_local_idx, :2].detach().cpu().numpy().astype(int) - int(atoms_offset)
                    true_atom_i, true_atom_j = int(true_pair[0]), int(true_pair[1])
                except Exception:
                    true_atom_i, true_atom_j = None, None

            # Store local candidate bond atoms/features before offsets move.
            try:
                local_bond_atom_pairs = bond_tensor[:, :2].detach().cpu().numpy().astype(int) - int(atoms_offset)
            except Exception:
                local_bond_atom_pairs = np.zeros((int(cand_bond_size), 2), dtype=int)

            try:
                local_bond_features = bond_tensor[:, 2:].detach().cpu().numpy().astype(float)
            except Exception:
                local_bond_features = np.zeros((int(cand_bond_size), 0), dtype=float)

            # BDE is expected at bond feature column 5 in your current modified model.
            # If absent, downstream CSV still exists with NaN values.
            if local_bond_features.ndim == 2 and local_bond_features.shape[1] > 5:
                bond_bde_values = local_bond_features[:, 5].astype(float)
            else:
                bond_bde_values = np.ones(int(cand_bond_size), dtype=float) * np.nan

            # Lower percentile means weaker / lower-BDE bond.
            bond_bde_percentiles = np.ones(int(cand_bond_size), dtype=float) * np.nan
            finite = np.where(np.isfinite(bond_bde_values))[0]
            if len(finite) == 1:
                bond_bde_percentiles[finite[0]] = 0.0
            elif len(finite) > 1:
                order = finite[np.argsort(bond_bde_values[finite])]
                vals = np.linspace(0.0, 1.0, len(order))
                bond_bde_percentiles[order] = vals
            ...
            product_smiles = None
            if product_smiles_list is not None and i < len(product_smiles_list):
                product_smiles = product_smiles_list[i]
            ...
            detail_scopes.append(
                {
                    ...
                    "num_bond_candidates": int(cand_bond_size),
                    "num_atom_candidates": int(atom_size),
                    "length": int(4 * cand_bond_size + atom_size),
                    "atoms_offset": int(atoms_offset),
                    "true_label": int(true_label),
                    "true_center_type": true_center_type,
                    "true_atom_i": true_atom_i,
                    "true_atom_j": true_atom_j,
                    "true_bond_local_idx": true_bond_local_idx,
                    "true_bond_change_type": true_bond_change_type,
                    "local_bond_atom_pairs": local_bond_atom_pairs,
                    "local_bond_features": local_bond_features,
                    "bond_bde_values": bond_bde_values,
                    "bond_bde_percentiles": bond_bde_percentiles,
                }
            )
        ...
        product_hiddens = torch.cat(product_hiddens, dim=0)
        cand_atoms_embeds, cand_bonds_embeds = self.get_center_embeds(
            product_atom_vecs,
            cand_bond_types,
            cand_bond_atom_idxs,
            cand_atom_atom_idxs,
        ...
        length_tensor = torch.LongTensor([length[2] for length in lengths]).to(device)
        rank_log_probs, log_probs = variable_likelihood(pad_logits, length_tensor)
        center_ranks = torch.argsort(rank_log_probs, descending=True, dim=1)

        # Keep the original output dimensionality of 10 columns for backward compatibility.
        eval_k = max(10, int(knum))
        bond_center_acc = np.zeros((len(bond_center_idxs), eval_k))
        atom_center_acc = np.zeros((len(atom_center_idxs), eval_k))
        top_10_acc = np.zeros((log_probs.shape[0] + len(skip_idxs), eval_k))

        for i in range(eval_k):
            for j in range(center_ranks.shape[0]):
                valid_len = int(lengths[j][2])
                top_slice = center_ranks[j, :min(i + 1, valid_len)]
                if labels[j] in top_slice:
                    top_10_acc[j, i] = 1
        ...
        candidate_details = []
        rank_scores_np = rank_log_probs.detach().cpu().numpy()
        log_probs_np = log_probs.detach().cpu().numpy()
        center_ranks_np = center_ranks.detach().cpu().numpy()
        pad_logits_np = pad_logits.detach().cpu().numpy()

        for j, scope in enumerate(detail_scopes):
            num_bond = int(scope["num_bond_candidates"])
            num_atom = int(scope["num_atom_candidates"])
            length = int(scope["length"])
            true_label = int(scope["true_label"])

            order = center_ranks_np[j, :length].astype(int)
            rank_pos = np.ones(length, dtype=int) * (length + 1)
            for r, cand_label in enumerate(order, start=1):
                if 0 <= cand_label < length:
                    rank_pos[cand_label] = r

            bde_values = scope["bond_bde_values"]
            bde_percentiles = scope["bond_bde_percentiles"]
            local_pairs = scope["local_bond_atom_pairs"]
            local_features = scope["local_bond_features"]

            for cand_label in range(length):
                row = {
                    "sample_id": scope["sample_id"],
                    "batch_index": int(j),
                    "product_smiles": scope["product_smiles"],
                    "candidate_label": int(cand_label),
                    "true_label": int(true_label),
                    "is_true_center": int(cand_label == true_label),
                    "true_center_type": scope["true_center_type"],
                    "true_atom_i": scope["true_atom_i"],
                    "true_atom_j": scope["true_atom_j"],
                    "true_bond_local_idx": scope["true_bond_local_idx"],
                    "true_bond_change_type": scope["true_bond_change_type"],
                    "candidate_rank": int(rank_pos[cand_label]),
                    "candidate_rank_score": float(rank_scores_np[j, cand_label]),
                    "candidate_log_prob": float(log_probs_np[j, cand_label]),
                    "candidate_logit": float(pad_logits_np[j, cand_label]),
                    "is_pred_top1": int(rank_pos[cand_label] <= 1),
                    "is_pred_top2": int(rank_pos[cand_label] <= 2),
                    "is_pred_top3": int(rank_pos[cand_label] <= 3),
                    "is_pred_top5": int(rank_pos[cand_label] <= 5),
                    "is_pred_top10": int(rank_pos[cand_label] <= 10),
                }

                if cand_label < num_bond * 4:
                    bond_local_idx = int(cand_label // 4)
                    bond_change_type = int(cand_label % 4)
                    pair = local_pairs[bond_local_idx]
                    row.update(
                        {
                            "candidate_type": "bond",
                            "candidate_bond_local_idx": bond_local_idx,
                            "candidate_bond_change_type": bond_change_type,
                            "candidate_atom_i": int(pair[0]),
                            "candidate_atom_j": int(pair[1]),
                            "candidate_bde": float(bde_values[bond_local_idx]) if np.isfinite(bde_values[bond_local_idx]) else np.nan,
                            "candidate_bde_percentile_in_mol": float(bde_percentiles[bond_local_idx]) if np.isfinite(bde_percentiles[bond_local_idx]) else np.nan,
                            "candidate_min_adjacent_bde": np.nan,
                            "candidate_min_adjacent_bde_percentile": np.nan,
                        }
                    )
                    if local_features.ndim == 2 and bond_local_idx < local_features.shape[0]:
                        feats = local_features[bond_local_idx]
                        for fidx, fval in enumerate(feats):
                            row[f"bond_feature_{fidx}"] = float(fval)
                        if len(feats) > 0:
                            row["bond_type_feature"] = float(feats[0])
                        if len(feats) > 1:
                            row["bond_conjugated_feature"] = float(feats[1])
                        if len(feats) > 2:
                            row["bond_ring_feature"] = float(feats[2])
                        if len(feats) > 3:
                            row["bond_aromatic_feature"] = float(feats[3])
                        if len(feats) > 5:
                            row["bde_feature_raw"] = float(feats[5])
                else:
                    atom_local_idx = int(cand_label - num_bond * 4)
                    adj = []
                    adj_pct = []
                    for bi, pair in enumerate(local_pairs):
                        if int(pair[0]) == atom_local_idx or int(pair[1]) == atom_local_idx:
                            if bi < len(bde_values) and np.isfinite(bde_values[bi]):
                                adj.append(float(bde_values[bi]))
                            if bi < len(bde_percentiles) and np.isfinite(bde_percentiles[bi]):
                                adj_pct.append(float(bde_percentiles[bi]))

                    row.update(
                        {
                            "candidate_type": "atom",
                            "candidate_bond_local_idx": np.nan,
                            "candidate_bond_change_type": np.nan,
                            "candidate_atom_i": atom_local_idx,
                            "candidate_atom_j": np.nan,
                            "candidate_bde": np.nan,
                            "candidate_bde_percentile_in_mol": np.nan,
                            "candidate_min_adjacent_bde": min(adj) if len(adj) > 0 else np.nan,
                            "candidate_min_adjacent_bde_percentile": min(adj_pct) if len(adj_pct) > 0 else np.nan,
                        }
                    )

                candidate_details.append(row)

        return top_10_acc, bond_center_acc, atom_center_acc, candidate_details
    ...
    def test_centers(self, product_embed_vecs, product_atom_vecs, product_trees, product_bond_tensors, product_graph_scopes):
        ...
        for i, tree in enumerate(product_trees):
            ...
            bond_tensor = product_bond_tensors[bonds_offset:bonds_offset+bond_size, :]
            one_dir_bond_tensor = torch.where(bond_tensor[:, -2] == 0)[0]
