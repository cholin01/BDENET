...
class EnsemblePredictions:
    ...
    def get_uncertainties(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        u_tot = self.shannon_entropy(torch.mean(probs, dim=0))
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        descriptions = self.descriptions
        if len(descriptions) != len(self.atom_ids):
            descriptions = [""] * len(self.atom_ids)
        ...
        if model_names is None:
            model_names = [f"model_{i}" for i in range(logits.shape[0])]
...
class GINEWithContextPooling(nn.Module):
    """
    Modified GINEConv operator + graph context pooling.
...
class SOMPredictor(nn.Module):
    ...
    @staticmethod
    def _expand_descriptions_to_atoms(batch: Data) -> List[str]:
        if not hasattr(batch, "batch"):
            return [""] * int(batch.num_nodes)

        graph_indices = batch.batch.detach().cpu().tolist()
        descriptions = getattr(batch, "description", None)

        if descriptions is None:
            return [""] * len(graph_indices)
        ...
        for graph_idx in graph_indices:
            if 0 <= graph_idx < len(descriptions):
                out.append(descriptions[graph_idx])
            else:
                out.append("")
    ...
    def predict(
        ...
        descriptions_atom = self._expand_descriptions_to_atoms(batch)
...
def _infer_model_name(path: str, index: int) -> str:
    ...
    for part in reversed(p.split(os.sep)):
